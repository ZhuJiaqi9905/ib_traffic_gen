...
int main(int argc, char **argv) {
  ...
  if (!exchange_metadata_with_client(client_sockfd, connections, num_qps,
                                     true)) {
...
static void print_usage(char *app) {
  ...
  fprintf(
      stderr,
      "  -i, --ib-port=<port>      use port <port> of IB device (default %d)\n",
      DEFAULT_IB_PORT);
  fprintf(
      stderr,
      "  -s, --size=<size>         size of message to exchange (default %d)\n",
      DEFAULT_MSG_SIZE);
  fprintf(stderr,
...
static bool parse_args(int argc, char **argv) {
  while (1) {
    ...
    int c = getopt_long(argc, argv, "p:d:i:s:q:b:cmh", long_options, NULL);
...
static bool wait_completion(int cli_sockfd) {
  ...
  return read_exact(cli_sockfd, buf, buf_size) == buf_size &&
         strcmp(buf, COMPLETE_MSG) == 0;
}
