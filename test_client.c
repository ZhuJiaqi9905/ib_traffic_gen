...
static bool gen_traffic(struct conn_context *connections, unsigned int num_qps,
                        unsigned int iters, unsigned int tx_depth,
                        bool use_event);
// Send experiment completion message to the server
static bool send_completion(int serv_sockfd);

static uint16_t server_port = DEFAULT_SERVER_PORT;
static char *ib_dev_name = NULL;
static int ib_port = DEFAULT_IB_PORT;
static unsigned int tot_area_size = 1073741824; //1G
...
int main(int argc, char **argv) {
  ...
  struct dev_context device = {.ib_dev_name = ib_dev_name,
                               .dev_port = ib_port,
                               .ctx = NULL,
                               .channel = NULL,
                               .pd = NULL,
  ...
  struct conn_context *connections =
      (struct conn_context *)malloc(num_qps * sizeof(struct conn_context));
  if (!connections) {
    goto destroy_device;
  }
  ...
  for (unsigned int i = 0; i < num_qps; i++) {
    ...
    connections[i].data_buf_size = tot_area_size;
  ...
  if (!exchange_metadata_with_server(server_sockfd, connections, num_qps,
                                     true)) {
  ...
  if (controller_ip && !notify_controller(controller_ip, controller_port,
                                          connections, num_qps)) {
...
static void print_usage(char *app) {
  ...
  fprintf(stderr,
          ...
          DEFAULT_IB_PORT);
  fprintf(stderr,
          "  -C, --controller=<ip>        IP address of the controller\n");
  fprintf(stderr,
          "  -P, --controller-port=<port> connect to port <port> of controller "
          "(default %d)\n",
  ...
  fprintf(stderr,
          ...
          DEFAULT_ITERS);
  fprintf(stderr,
          "  -q, --qp=<num of qp's>       number of queue pairs (default %d)\n",
          DEFAULT_NUM_QPS);
  fprintf(stderr,
          "  -t  --tx-depth=<dep>         size of tx queue of each queue pair "
          "(default %d)\n",
          DEFAULT_TX_DEPTH);
  fprintf(
      stderr,
      "  -l, --inline                 inline message with the work request\n");
  fprintf(stderr, "  -e, --events                 sleep on CQ events\n");
  fprintf(stderr, "  -c, --chk                    validate received buffer\n");
  fprintf(stderr, "  -m, --multi-gid              use multiple GIDs associated "
                  "with the IB interface\n");
...
static bool parse_args(int argc, char **argv) {
  while (1) {
    ...
    case 's':
      tot_area_size = (unsigned int)strtoul(optarg, NULL, 0);
      break;
...
static bool gen_traffic(struct conn_context *connections, unsigned int num_qps,
  ...
  for (unsigned int i = 0; i < num_qps; i++) {
    printf("QP %u: post %u requests, complete %u requests", i,
           connections[i].post_reqs, connections[i].complete_reqs);
...
static bool send_completion(int serv_sockfd) {
  ...
  return write_exact(serv_sockfd, buf, buf_size) == buf_size;
}


void post_write_with_addr
...
static void mytest(struct conn_context *ctx) {
  ...
  for(int block = 1024; block <= tot_area_size; block <<= 1) {
    ...
    for(int addr = 0; addr < tot_area_size; addr += block) {
      post_write_with_addr(ctx, addr, addr + block);
      while (1) {
        int ne = ibv_poll_cq(cq, 1, &wc);
        if (ne < 0) {
          fprintf(stderr, "fail asdjlkf");
